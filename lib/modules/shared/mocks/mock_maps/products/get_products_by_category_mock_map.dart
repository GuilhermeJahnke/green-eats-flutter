class GetProductsByCategoryMockMap {
  const GetProductsByCategoryMockMap._();

  static const Map<String, dynamic> body = {
    'id': '1521422424',
  };

  static const dynamic response = [
    {
      'id': '52132d-235-s312',
      'title': 'Marmita de carne',
      'description':
          'Sed posuere molestie facilisis. Vivamus sodales commodo orci a bibendum. Ut sit amet ipsum nisi. Quisque nec semper diam. Duis porttitor viverra felis vitae viverra. Nunc justo turpis, eleifend ac congue non, posuere id lacus. Proin sit amet bibendum nisi. Sed eu faucibus libero.',
      'price': 23.54,
      'weight': 0.200,
      'discount': 10,
      'imagePath':
          'https://img.freepik.com/vetores-gratis/ilustracao-de-preparacao-de-refeicao-de-design-plano-desenhado-a-mao_23-2149350982.jpg',
    },
    {
      'id': '5213w3523-51-6434-2',
      'title': 'Marmita de peixe',
      'description':
          'Sed posuere molestie facilisis. Vivamus sodales commodo orci a bibendum. Ut sit amet ipsum nisi. Quisque nec semper diam. Duis porttitor viverra felis vitae viverra. Nunc justo turpis, eleifend ac congue non, posuere id lacus. Proin sit amet bibendum nisi. Sed eu faucibus libero.',
      'price': 23.54,
      'weight': 0.200,
      'discount': 10,
      'imagePath':
          'https://img.freepik.com/vetores-gratis/ilustracao-de-comida-kawaii-desenhada-a-mao_52683-84890.jpg',
    },
    {
      'id': '523820-gsae124290-as421',
      'title': 'Marmita de frango',
      'description':
          'Sed posuere molestie facilisis. Vivamus sodales commodo orci a bibendum. Ut sit amet ipsum nisi. Quisque nec semper diam. Duis porttitor viverra felis vitae viverra. Nunc justo turpis, eleifend ac congue non, posuere id lacus. Proin sit amet bibendum nisi. Sed eu faucibus libero.',
      'price': 23.54,
      'weight': 0.200,
      'discount': 10,
      'imagePath':
          'https://img.freepik.com/vetores-premium/ilustracao-da-caixa-de-bento-desenhada-a-mao_23-2148845283.jpg',
    }
  ];
}
